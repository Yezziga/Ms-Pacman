...
import pacman.controllers.examples.PacmanAI;
...
public class DecisionTree extends Controller<MOVE> {
	...
	public Node generateTree(Dataset dataset, ArrayList<String> attributeList) {
		...
		// step 3
			...
			node = new Node(getMajorityClass().toString()); // make node as a leaf node labeled with the majority class
															// in D.
			return node;
		}
		// step 4
	...
	/**
	 * Calculates the expected information (entropy) of the dataset. Gives the
	 * average amount of info needed to identify the class label of a tuple in
	 * dataset
	 * 
	 * @param attribute
	 * @return
	...
	public STRATEGY searchForStrategy(Node node, DataTuple tuple) {
		...
		if (node.isLeaf()) {
			strat = STRATEGY.valueOf(node.getLabel());
		} else {
			String nodeValue = tuple.discretizeAttrValue(node.getLabel());
			TreeMap tree = node.getChildren();

			Node nextNode = (Node) tree.get(nodeValue);
			if (nextNode == null) {
				nextNode = (Node) tree.get("default");
			}
			strat = searchForStrategy(nextNode, tuple);
		}
		return strat;
	}

	public STRATEGY getFinalStrategy(Game game) {
		DataTuple tuple = new DataTuple(game, null);
		return searchForStrategy(root, tuple);
	}

	@Override
	public MOVE getMove(Game game, long timeDue) {
		PacmanAI pacman = new PacmanAI();
		MOVE pacmanResult = pacman.getMove(game, timeDue);
		MOVE nextMove = null;
		...
		case EATPILLS:
			...
			nextMove = game.getNextMoveTowardsTarget(game.getPacmanCurrentNodeIndex(),
					game.getClosestNodeIndexFromNodeIndex(game.getPacmanCurrentNodeIndex(),
							game.getAllAvailablePillsAndPowerPills(), DM.PATH),
					DM.PATH);
		...
		case CHASE : 
			...
			nextMove = game.getNextMoveTowardsTarget(game.getPacmanCurrentNodeIndex(), game.getGhostCurrentNodeIndex(ghostA), DM.PATH);
			
			break;
		case FLEE :
			int minDistFlee = Integer.MAX_VALUE;
			GHOST ghostB = null;
			...
			nextMove = game.getNextMoveAwayFromTarget(game.getPacmanCurrentNodeIndex(), game.getGhostCurrentNodeIndex(ghostB), DM.PATH);
		...
		if(pacmanResult == nextMove) {
			...
			System.err.println("Incorrect prediction. Pacman's decision: " + pacmanResult);
		}
		return nextMove;
	}
